...
export default async function AdminDashboard() {
 ...
 // Subscription metrics from RevenueCat proxy (server-side)
 ...
 try {
  ...
  if (revenuecatApiKey) {
   ...
   const rcRes = await fetch(
    `https://api.revenuecat.com/v1/projects/${appId}/subscribers`,
    {
     headers: {
   ...
   if (rcRes.ok) {
    ...
    trialCount = data.subscribers?.filter(
     (s: any) => s.subscription?.status === 'trial'
    ).length ?? 0;
    premiumCount = data.subscribers?.filter(
     (s: any) => s.subscription?.status === 'active'
    ).length ?? 0;
   }
  }
 } catch {
  // RevenueCat unavailable — show 0
 }
 ...
 return (
  <div>
   ...
   <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
    ...
    <MetricCard label="Active Subscribers" value={subscriberCount} />
    <MetricCard label="In Trial" value={trialCount} />
    <MetricCard label="Premium Active" value={premiumCount} />
   </div>
   ...
   <div className="bg-white rounded-lg border border-gray-200 p-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
     ...
     <QuickAction href="/admin/audit" label="Audit Log" description="Track admin actions" />
     <QuickAction href="/admin/subscriptions" label="Subscriptions" description="RevenueCat subscriber management" />
    </div>
   </div>
  </div>
 );
...
function QuickAction({ href, label, description }: { href: string; label: string; description: string }) {
 return (
  <a
   href={href}
   className="flex flex-col p-3 rounded-lg border border-gray-200 hover:border-primary hover:bg-gray-50 transition-colors"
  >
